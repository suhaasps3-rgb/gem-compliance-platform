...
# Add mismatch detection variables for GSTR-3B and Debarment
mismatch_vars = """  const miiNameMismatch = !!(bidderDetails?.name && miiResult?.extracted?.entity_name && !miiResult.extracted.entity_name.toLowerCase().includes(bidderDetails.name.toLowerCase().replace(' private limited', '').replace(' corp', '')));
  const isMiiMismatch = miiNameMismatch;

  const gstr3bNameMismatch = !!(bidderDetails?.name && gstr3bResult?.extracted?.entity_name && !gstr3bResult.extracted.entity_name.toLowerCase().includes(bidderDetails.name.toLowerCase().replace(' private limited', '').replace(' corp', '').replace(' llc', '')));
  
  const debarmentNameMismatch = !!(bidderDetails?.name && debarmentResult?.extracted?.entity_name && !debarmentResult.extracted.entity_name.toLowerCase().includes(bidderDetails.name.toLowerCase().replace(' private limited', '').replace(' corp', '').replace(' llc', '')));"""

content = re.sub(r'  const miiNameMismatch = [^\n]+\n  const isMiiMismatch = miiNameMismatch;', mismatch_vars, content)

# Replace the GSTR-3B panel
gstr3b_regex = r'\{gstr3bResult && \([\s\S]*?\}\s*</Panel>\s*\)\}'
...
debarment_regex = r'\{debarmentResult && \([\s\S]*?\}\s*</Panel>\s*\)\}'
