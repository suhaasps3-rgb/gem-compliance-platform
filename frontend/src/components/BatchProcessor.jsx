...
export default function BatchProcessor({ onClose }) {
  ...
  useEffect(() => {
    ...
    if (status === 'PROCESSING' && batchId) {
      interval = setInterval(async () => {
        try {
          const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000' : '')}/api/v1/batch/${batchId}/status`);
          if (res.ok) {
            const data = await res.json();
            
            // we also need the full batch details to get bidder info
            const resDetails = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000' : '')}/api/v1/batch/${batchId}`);
            if (resDetails.ok) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:8000' : '')}/api/v1/batch/upload`, {
        method: 'POST',
        body: fd
      });
